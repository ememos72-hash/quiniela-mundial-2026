...
const LigaMatchCard = ({ match, userId }) => {
          ...
          {localPred && (
            <div style={{ background: 'var(--navy)', borderRadius: 10, padding: '10px 14px 14px', marginTop: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
                ...
                <span style={{ marginLeft: 'auto', fontSize: 10, color: 'rgba(255,255,255,0.4)' }}>opcional</span>
              </div>
      ...
      {match.result && prediction && prediction.pointsAwarded > 0 && (
        <div style={{ textAlign: 'center', marginTop: 8 }}>
          <span style={{ background: '#dcfce7', color: '#15803d', borderRadius: 20, padding: '3px 12px', fontSize: 12, fontWeight: 700 }}>
            +{prediction.pointsAwarded} pts ganados
          </span>
        </div>
      )}
    </div>
...
const LigaSection = ({ userId, userProfile }) => {
  ...
  // Cargar partidos si tiene acceso
  useEffect(() => {
          ...
          <>
            ...
            {!loadingMatches && matches.length === 0 && (
              <div style={{ textAlign: 'center', padding: 24, color: 'var(--text-muted)', fontSize: 13 }}>
                Los partidos se publicarán pronto. ¡Estate atento!
              </div>
            )}
            ...
            {crMatches.length > 0 && (
              <>
                ...
                {crMatches.map(m => (
                  <LigaMatchCard key={m.id} match={m} userId={userId} />
                ))}
              </>
            )}
            ...
            {mxMatches.length > 0 && (
              <>
                <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '14px 0 8px' }}>
                  🇲🇽 México
                </div>
                {mxMatches.map(m => (
                  <LigaMatchCard key={m.id} match={m} userId={userId} />
            ...
            {matches.length > 0 && <LigaRanking currentUserId={userId} />}
          </>
        )}
      </div>
    </div>
